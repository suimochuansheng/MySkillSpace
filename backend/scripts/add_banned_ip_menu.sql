...
SET @system_menu_id = (SELECT id FROM auth_system_menu WHERE name = '系统管理' LIMIT 1);

-- 2. 插入"封禁IP管理"菜单（菜单类型）
INSERT INTO auth_system_menu (
    name,
    path,
    component,
    menu_type,
...
-- 4. 将新菜单分配给管理员角色
-- 查找管理员角色ID
SET @admin_role_id = (SELECT id FROM auth_system_role WHERE code = 'admin' LIMIT 1);

-- 将"封禁IP管理"菜单分配给管理员
INSERT INTO auth_system_role_menus (role_id, menu_id)
...
FROM auth_system_menu m
LEFT JOIN auth_system_menu p ON m.parent_id = p.id
