...
class UserChangeForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ('email', 'username', 'password', 'plain_password', 'avatar', 'phonenumber',
...
class CustomUserAdmin(BaseUserAdmin):
    ...
    # 列表页显示的字段
    list_display = ('email', 'username', 'plain_password_display', 'is_active', 'is_staff', 'date_joined')
...
class OperationLogAdmin(admin.ModelAdmin):
    """操作日志Admin"""
    list_display = (
        'id', 'username', 'module', 'action', 'status',
    ...
    readonly_fields = (
        'user', 'username', 'module', 'action', 'description',
        'method', 'url', 'ip_address', 'user_agent',
        'request_params', 'response_data', 'status', 'error_msg',
        'created_at', 'duration'
    )
    ordering = ('-created_at',)
    ...
    fieldsets = (
        ...
        ('响应信息', {
            'fields': ('status', 'response_data', 'error_msg')
        }),
        ('时间信息', {
...
class LoginLogAdmin(admin.ModelAdmin):
    """登录日志Admin"""
    list_display = (
        'id', 'username', 'ip_address', 'login_location',
        'browser', 'os', 'status', 'msg', 'login_time'
    )
    list_filter = ('status', 'login_time')
    search_fields = ('username', 'ip_address', 'login_location')
    readonly_fields = (
        'username', 'ip_address', 'login_location',
        'browser', 'os', 'device', 'status', 'msg', 'login_time'
    )
    ordering = ('-login_time',)
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('username', 'ip_address', 'login_location')
        }),
        ('设备信息', {
            'fields': ('browser', 'os', 'device')
        }),
        ('状态信息', {
            'fields': ('status', 'msg', 'login_time')
        }),
    )
