...
class AITaskAdmin(admin.ModelAdmin):
    """AI 任务管理"""
    list_display = [
        'task_id_short', 'user_display', 'prompt_short',
    ...
    fieldsets = (
        ('任务信息', {
            'fields': ('task_id', 'celery_task_id', 'status', 'ws_url')
        }),
        ('用户信息', {
...
class ChatRecordAdmin(admin.ModelAdmin):
    """对话记录管理"""
    list_display = [
        'id', 'session_id_short', 'user_display',
        'role', 'content_short', 'created_at', 'toggle_hidden_button'
    ]
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('session_id', 'user', 'role')
        }),
        ('对话内容', {
            'fields': ('content',)
        }),
        ('状态信息', {
            'fields': ('is_hidden', 'created_at')
        }),
    )
